...
export function useAPI<T>(endpoint: string | null, options: UseAPIOptions = {}) {
  const { method, data, customHeaders, useAuthBaseUrl, fetchOptions, useAuth, ...swrOptions } = options;
  ...
  const {
    ...
    mutate,
    isLoading,
  } = useSWR<T>(endpoint, endpoint ? () => apiRequest(endpoint, fetcherOptions) : null, {
  ...
  const addItem = (newItem: T extends any[] ? T[0] : T) => {
    mutate((currentData: T | undefined) => {
      ...
      const arrayData = (Array.isArray(currentData) ? currentData : []) as ArrayResponse<T>;
  ...
  const putItem = <Type>(id: Type, key: keyof (T extends any[] ? T[0] : T), value: any) => {
    mutate((currentData: T | undefined) => {
      if (!currentData) {
        return [] as T;
      }
      const arrayData = (Array.isArray(currentData) ? currentData : []) as ArrayResponse<T>;
      return arrayData.map((item: any) => (item.id === id ? { ...item, [key]: value } : item)) as T;
    }, false);
  };
